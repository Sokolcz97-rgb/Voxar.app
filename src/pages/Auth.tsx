...
const Auth = () => {
  ...
  const handleGoogle = async () => {
    ...
    if (result.error) {
      toast({ title: t("auth.googleFailed"), description: result.error.message, variant: "destructive" });
      return;
    }
    ...
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden">
      ...
      <div className="w-full max-w-md animate-scale-in">
          ...
          <Tabs defaultValue="signin" className="w-full">
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignUp} className="space-y-4 mt-6">
                <div className="space-y-2">
                  <Label htmlFor="name-up">{t("auth.displayName")}</Label>
        ...
        <p className="text-center text-xs text-muted-foreground mt-6">
          <Link to="/" className="hover:text-primary transition-colors">{t("auth.backHome")}</Link>
        </p>
      </div>
    </div>
  );
