...
import { Bell, BellRing, CalendarDays, CheckCheck, CircleAlert, Loader2, Megaphone, Radio, Trash2 } from "lucide-react";
...
export function CommunityNotifications() {
  const { notifications, unreadCount, loading, error, markRead, markAllRead, remove } = useVoxNotifications();
  const [filter, setFilter] = useState<Filter>("all");

  const visible = useMemo(() => filter === "unread" ? notifications.filter((item) => !item.is_read) : notifications, [notifications, filter]);
  ...
  const openItem = async (item: VoxNotification) => {
    ...
    } else if (eventId || item.type.startsWith("event_")) {
  ...
  const removeItem = async (event: React.MouseEvent, id: string) => {
    ...
    <div className="sv-feature-page sv-notifications-page">
      <div className="sv-feature-toolbar sv-notifications-toolbar">
        ...
        <div className="sv-notifications-heading-actions">
          <div className={`sv-notification-count${unreadCount ? " has-unread" : ""}`}><BellRing /><span><strong>{unreadCount}</strong><small>nepřečtených</small></span></div>
          {unreadCount > 0 && <button className="sv-hud-button secondary" onClick={() => void markEverything()}><CheckCheck /> Přečíst vše</button>}
        </div>
      </div>

      <div className="sv-notification-controls">
        <button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}>Všechna <b>{notifications.length}</b></button>
        <button className={filter === "unread" ? "active" : ""} onClick={() => setFilter("unread")}>Nepřečtená <b>{unreadCount}</b></button>
      </div>

      {loading ? (
        <div className="sv-feature-loading"><Loader2 className="animate-spin" /> Načítám oznámení…</div>
      ) : error ? (
        <div className="sv-feature-empty"><Bell /><strong>Centrum oznámení zatím není dostupné</strong><span>{error}</span></div>
      ) : visible.length === 0 ? (
        ...
        <div className="sv-notification-list">
          {visible.map((item) => (
            <article className={`sv-notification-row${item.is_read ? "" : " unread"}`} key={item.id} onClick={() => void openItem(item)}>
              ...
              <div className="sv-notification-actions">
                <button type="button" className="danger" title="Odstranit" onClick={(event) => void removeItem(event, item.id)}><Trash2 /></button>
              </div>
            </article>
          ))}
        </div>
