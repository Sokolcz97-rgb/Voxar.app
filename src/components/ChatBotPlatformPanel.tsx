...
export function ChatBotPlatformPanel({ platform }: { platform: ChatPlatform }) {
  ...
  const addChannel = async () => {
    ...
    const { error } = await supabase
      .from("chat_bot_channels")
      .insert({ platform, handle: h, display_name: newHandle.trim() });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-primary text-glow flex items-center gap-2">
            <Icon className="h-4 w-4" /> {label} chat bot
          </p>
          <h2 className="font-display font-black text-2xl md:text-3xl mt-2">
            {label} chat — automod, anti-scam, příkazy
          </h2>
          <p className="text-muted-foreground mt-1 text-sm max-w-2xl">
            Bot se přihlásí pod sdíleným bot účtem a moderuje chat v reálném čase. Přidej kanály a nastav je individuálně.
          </p>
        </div>
        <Button onClick={() => setAddOpen(true)} className="gap-2">
          <Plus className="h-4 w-4" /> Přidat {label} kanál
        </Button>
      </div>
      ...
      {!loading && channels.length === 0 ? (
        <Card className="glass border-border p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1 mb-4">
            Přidej {label} kanál, do kterého má bot vstoupit.
          </p>
          <Button onClick={() => setAddOpen(true)} className="gap-2"><Plus className="h-4 w-4" /> Přidat</Button>
        </Card>
      ) : null}
      {channels.map((c) => (
      ...
      <Card className="glass border-border p-5 text-sm text-muted-foreground">
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Přidat {label} kanál</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                className="mt-1"
                placeholder={platform === "twitch" ? "např. sokolcze1997" : "např. @sokolcze nebo UC..."}
                value={newHandle}
