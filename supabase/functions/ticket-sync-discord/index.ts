...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Load ticket via user RLS (own ticket OR tickets:view_all)
    ...
    // Service-role client for config + queue insert (config is restricted by RLS)
    ...
    const { data: cfg } = await admin
      ...
      .select('mirror_enabled, sync_channel_id, sync_webhook_url')
      .limit(1)
      .maybeSingle();

    if (!cfg?.mirror_enabled || (!cfg.sync_channel_id && !cfg.sync_webhook_url)) {
      return new Response(JSON.stringify({ skipped: true, reason: 'mirror disabled' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    // Load actor profile (the user triggering this event)
    ...
    const authorName =
      authorProfile?.display_name || authorProfile?.username || 'Uživatel';
    const actorName =
      actorProfile?.display_name || actorProfile?.username || 'Uživatel';
    ...
    // Send via webhook if configured (instant), else queue for external bot
    if (cfg.sync_webhook_url) {
      const res = await fetch(cfg.sync_webhook_url, {
        method: 'POST',
    ...
    // Queue for the external Discord bot
    const { error: qErr } = await admin.from('bot_outbound_queue').insert({
